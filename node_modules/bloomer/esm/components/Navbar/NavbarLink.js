import * as tslib_1 from "tslib";
import * as classNames from 'classnames';
import * as React from 'react';
import { getActiveModifiers, removeActiveModifiers, withHelpersModifiers, } from './../../bulma';
import { getHTMLProps } from './../../helpers';
export function NavbarLink(_a) {
    var _b = _a.tag, tag = _b === void 0 ? 'a' : _b, render = _a.render, props = tslib_1.__rest(_a, ["tag", "render"]);
    var className = classNames('navbar-link', tslib_1.__assign({}, getActiveModifiers(props)), props.className);
    if (render)
        return render(tslib_1.__assign({}, props, { className: className }));
    var HTMLProps = getHTMLProps(props, removeActiveModifiers);
    return React.createElement(tag, tslib_1.__assign({}, HTMLProps, { className: className }));
}
var HOC = withHelpersModifiers(NavbarLink);
export default HOC;
//# sourceMappingURL=NavbarLink.js.map